...
function [berErrorTrMulti,berErrorTeMulti,berErrorTrBin,berErrorTeBin]=learnMultinomLogisticRegression(y,X,Basis,Deg)
...
for s=1:length(seed)
    ...
    for k = 1:K
        ...
        beta = multinomLogisticRegression(yTr, XTr);  

        % Sample Cross Validation Error
        yPred=multinomClassPredLogistReg(XTr,beta);
        berErrorCVtrMulti(k)=ber(yTr,yPred,'Multiclass');
        berErrorCVtrBin(k)=ber(yTr,yPred,'Binary');
        yPred=multinomClassPredLogistReg(XTe,beta);
